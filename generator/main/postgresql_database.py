...
class OracleDatabase(AbstractDatabase):
    ...
    def generate_create_table(self, table, hive_database_name, table_location, create_table_template, partition,
        ...
        table_string = self.get_template(create_table_template).format(table_name=table_name, fields=fields,
                                                                       partition=partition,
                                                                       location=location)
        return table_string
    ...
    def generate_select_string(self, table_part, jira_database_name,
        ...
        new_fields = []
        found_partition = self.define_partition(all_fields)
        for f in all_fields:
    ...
    def generate_main(self, hive_database_name, source_database_name, load_type, partition_folder, part_name,
        ...
        for i in range(0, len(self.tables)):
            ...
            if i < len(self.tables) - 1:
                ...
                next_item = None
            props, wf = self.generate_part_for_properties_and_workflow(partition_folder, part_name,
                                                                       self.tables[i]["table"],
                                                                       self.tables[i]["table"],
